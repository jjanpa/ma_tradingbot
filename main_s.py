    ...
    def buy_ticker(self, list_ticker):
        try:
            ...
            if self.volume_buy >= self.margin_money:
                ...
                temp = pd.DataFrame(columns=self.column_trading, index=[0], data=0.0)
                # temp = pd.DataFrame(columns=self.column_trading)
                # pd.concat([temp,pd.Series(dtype=float, name=0)])   #append 대체
                ...
                self.list_trade = pd.concat([self.list_trade,temp])   #append 대체
        ...
        try:
            ...
            if self.flag_done_init == self.num_ticker:  #9시 정각 모든 티커가 셀링 되었을 때
                ...
                try:
                    ...
                    tmp_data = pd.DataFrame(columns=[],index=[0], data=0.0)
                    # tmp_data = pd.DataFrame()
                    # pd.concat([tmp_data,pd.Series(dtype=float, name=0)])    #replace append
                    ...
                    self.result = pd.concat([self.result,tmp_data],ignore_index=True) #replace append
